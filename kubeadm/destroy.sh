#!/usr/bin/env bash
# Tear down the entire kubeadm lab. Optional --keep-base preserves the base
# image (saves a re-download on the next ./provision.sh).
...
read -rp "Destroy all 5 kubeadm VMs, their disks, and the 'kubeadm' network? [y/N] " ans
...
if [[ -d "${KUBEADM_DIR}/.bootstrap" ]]; then
  log "Removing local bootstrap artifacts"
  rm -rf "${KUBEADM_DIR}/.bootstrap"
fi
if [[ -f "${KUBEADM_DIR}/admin.conf" ]]; then
  rm -f "${KUBEADM_DIR}/admin.conf"
fi
