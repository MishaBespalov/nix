#!/usr/bin/env bash
# Download the Debian 12 cloud image once. Idempotent — skips if present.
...
# Stage to /tmp so a partial download doesn't pollute the libvirt pool.
TMP_IMAGE="/tmp/kthw-debian12-base.qcow2.$$"
